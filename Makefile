...
load-local:
	docker run --cpus=$(DOCKER_YCSB_CPUS) --rm -it --network=$(DOCKER_NETWORK) ycsb load riak -P workloads/workloada -p threads=$(YCSB_THREADS) -p target=$(YCSB_TARGET) -p recordcount=$(YCSB_RECORD_COUNT) -p riak.hosts=$(YCSB_RIAK_HOSTS) -p riak.debug=true
load-remote:
	docker run --cpus=$(DOCKER_YCSB_CPUS) --rm -it --network=default ycsb load riak -P workloads/workloada -p threads=$(YCSB_THREADS) -p target=$(YCSB_TARGET) -p recordcount=$(YCSB_RECORD_COUNT) -p riak.hosts=$$(cd ansible; ansible-inventory --list -i scaleway/inventory.yml all | grep -v INFO | jq '._meta."hostvars" | .[] | .public_ipv4' | xargs | tr ' ' ',') -p riak.debug=true

run: run$(TARGET_SUFFIX)
run-local:
	docker run --cpus=$(DOCKER_YCSB_CPUS) --rm -it --network=$(DOCKER_NETWORK) ycsb run riak -P workloads/workloada -p threads=$(YCSB_THREADS) -p target=$(YCSB_TARGET) -p recordcount=$(YCSB_RECORD_COUNT) -p operationcount=$(YCSB_OPERATION_COUNT) -p riak.hosts=$(YCSB_RIAK_HOSTS) -p riak.debug=true
run-remote:
	...
	docker run --cpus=$(DOCKER_YCSB_CPUS) --rm -it --network=default ycsb load riak -P workloads/workloada -p threads=$(YCSB_THREADS) -p target=$(YCSB_TARGET) -p recordcount=$(YCSB_RECORD_COUNT) -p operationcount=$(YCSB_OPERATION_COUNT) -p riak.hosts=$(YCSB_RIAK_HOSTS) -p riak.debug=true
...
cluster-info-remote:
	cd ansible; ansible -i scaleway/inventory.yml riak_1 -m shell -a "riak-admin cluster status"
...
show-riak-config-remote:
	cd ansible; ansible -m shell -a 'riak config effective' -i scaleway/inventory.yml riak_1
